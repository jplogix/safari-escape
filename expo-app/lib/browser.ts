...
export async function setBrowserPreference(pref: BrowserPreference): Promise<void> {
  try {
...
export async function openDestination(url: string): Promise<BrowserHandoffResult> {
  try {
    const pref = await getBrowserPreference();
    const targetURL = pref === "chrome" ? url.replace(/^https?:\/\//, "googlechromes://") : url;
