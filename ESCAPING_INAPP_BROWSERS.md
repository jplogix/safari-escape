...
- **How it works:** It forces the OS to immediately open the Shortcuts app, run a 1-action script that accepts a URL, and passes it strictly to Safari. This breaks out of the Instagram sandbox entirely.
- **UX Enhancement:** Since users need the `"OpenSafari"` shortcut installed, we placed a subtle, styled iCloud installer link directly below the button (`https://www.icloud.com/shortcuts/2ae0484bf4454ef2a8fc413012ebc967`).
...
- User taps "Tap to open in Chrome" (`googlechromes://`).
- Instagram successfully hands off the session to the native Chrome app.
- Once the page loads natively in Chrome, our script detects the Chrome user agent (`CriOS`).
- It instantly attempts to re-fire `x-safari-https://` to bridge the user automatically from Chrome directly into Safari.
- **Chrome Fallback:** If Chrome blocks the cross-browser jump sequence, the script gracefully aborts the escape and simply loads the target destination URL inside Chrome natively, ensuring the user gets to the content outside of Instagram.